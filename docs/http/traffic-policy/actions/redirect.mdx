...
[Traffic Policy](/docs/http/traffic-policy/index.mdx) for redirecting to a new `version` with added `id` param.
...
```shell
curl http://example.ngrok.app/api/v1/users/42
```

_Result:_

- _Url Rewritten_
- _Headers added to response_
...
```shell
curl http://example.ngrok.app/api/v1
```
...
```shell
curl http://example.ngrok.app/api/v1/foobar
```
